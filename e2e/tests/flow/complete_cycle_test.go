...
var _ = Describe("Complete cycle", func() {
	...
	quorumSignTag := func(tag string) {
		testutil.RunSucceedCommand(
			testDir,
			"git",
			"signatures", "add", "--key", pgpKeyEmailTL, tag,
		)

		testutil.RunSucceedCommand(
			testDir,
			"git",
			"signatures", "add", "--key", pgpKeyEmailPM, tag,
		)
	}
	...
	quorumSignCommit := func(_ string) {
		testutil.RunSucceedCommand(
			testDir,
			"git",
			"signatures", "add", "--key", pgpKeyEmailTL, branchName, // TODO
		)

		testutil.RunSucceedCommand(
			testDir,
			"git",
			"signatures", "add", "--key", pgpKeyEmailPM, branchName, // TODO
		)
	}
