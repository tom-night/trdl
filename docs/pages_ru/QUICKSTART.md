...
где `developer.pgp`, полученный в результате вызова команды `gpg --armor --output developer.pgp --export developer@trdl.dev` (подробнее про экспорт с gpg можно почитать в [документации утилиты](https://www.gnupg.org/gph/en/manual/x56.html#AEN64)):
```shell
